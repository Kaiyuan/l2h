...
1. 构建所有架构的二进制文件（x86_64、aarch64、armv7）
2. 生成 SHA256 校验文件
3. 创建 GitHub Release（使用标签名称）
4. 上传所有构建产物到 Release（包括二进制文件和校验文件）
5. 自动生成 Release 说明，包含下载和安装指南

**重要**：Release 只有在推送标签时才会创建。PR 和手动触发只会构建，不会创建 Release。
